...
use solana_sdk::{
    ...
    pubkey,
    pubkey::Pubkey,
    transaction::Transaction, signature::Keypair, account::Account
};
...
fn get_prgram_accounts(rpc_client: &RpcClient, program_id: &Pubkey, memcmps: Vec<RpcFilterType>) -> Vec<(Pubkey, Account)>{
...
fn buy(rpc_client: &RpcClient, usr: &Keypair, program_id: &Pubkey, args: &Vec<String>) {
    ...
    let (product_pubkey, _) = Pubkey::find_program_address(&[&store_pubkey.to_bytes(), product.as_bytes()], program_id);
...
fn make_store(rpc_client: &RpcClient, usr: &Keypair, program_id: &Pubkey, args: &Vec<String>) {
    ...
    let rent_exemption = rpc_client.get_minimum_balance_for_rent_exemption(50 + 32).unwrap();


    let (account_pubkey, _) = Pubkey::find_program_address(&[store.name_of_store.as_bytes()], program_id);
...
fn add_to_store(rpc_client: &RpcClient, usr: &Keypair, program_id: &Pubkey, args: &Vec<String>) {
    ...
    let (account_pubkey, _) = Pubkey::find_program_address(&[&store_pubkey.to_bytes(), name_of_product.as_bytes()], program_id);
    ...
    let rent_exemption = rpc_client.get_minimum_balance_for_rent_exemption(32 + 32 + 50 + 8 + 8).unwrap();
...
fn delete_from_store(rpc_client: &RpcClient, usr: &Keypair, program_id: &Pubkey, args: &Vec<String>) {

    let name_of_store = String::from(&args[2]);
    let name_of_product = String::from(&args[3]);
    let (store_pubkey, _) = Pubkey::find_program_address(&[name_of_store.as_bytes()], program_id);
    let (product_pubkey, _) = Pubkey::find_program_address(&[&store_pubkey.to_bytes(), name_of_product.as_bytes()], program_id);
...
fn main() {
    ...
    let usr = read_keypair_file("/home/gohnny/.config/solana/id.json").unwrap();
    let program = pubkey!("J9cFnPQkWmLsXWEoARkdB9QJQksnMD4Bk76ptppLpPZU");
