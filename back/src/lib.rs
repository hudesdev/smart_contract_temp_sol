...
fn buy(accounts: &[AccountInfo]) {
    ...
    product.owner = *from;
    product.serialize(&mut *product_info.try_borrow_mut_data().unwrap()).unwrap();
}

fn make_store(program_id: &Pubkey, accounts: &[AccountInfo], store: &Store, rent_exemption: u64)  {
    let usr = &store.owner;

    let (account_pubkey, bump) = Pubkey::find_program_address(&[store.name_of_store.as_bytes()], program_id);
    ...
    invoke_signed(&instruction, accounts, &[&[store.name_of_store.as_bytes(), &[bump]]]).unwrap();
    
    let account = next_account_info(&mut accounts.into_iter()).unwrap();
    store.serialize(&mut *account.try_borrow_mut_data().unwrap()).unwrap();
}

fn add_to_store(program_id: &Pubkey, accounts: &[AccountInfo], product: &Product, rent_exemption: u64)  {
    ...
    let store = Store::deserialize(&mut store_info.data.borrow_mut().as_ref()).expect("Cannot deserialize store account");
    ...
    let (_, bump) = Pubkey::find_program_address(&[&store_pubkey.to_bytes(), product.name.as_bytes()], program_id);
    ...
    invoke_signed(&instruction, accounts, &[&[&store_pubkey.to_bytes(), product.name.as_bytes(), &[bump]]]).unwrap();
...
fn delete_from_store(accounts: &[AccountInfo])  {
    ...
    **owner_info.lamports.borrow_mut() = owner_info.lamports().checked_add(product_info.lamports()).unwrap();
...
fn close(accounts: &[AccountInfo]) {
    ...
    **owner_info.lamports.borrow_mut() = owner_info.lamports().checked_add(store_info.lamports()).unwrap();
...
fn wipe(accounts: &[AccountInfo]) {
    ...
    **owner_info.lamports.borrow_mut() = owner_info.lamports().checked_add(account_info.lamports()).unwrap();
...
pub fn process_instruction (
    ...
    match command {
        Command::MakeStore(store, rent_exemption) => make_store(&program_id, accounts, &store, rent_exemption),
        Command::Buy => buy(accounts),
        Command::AddToStore(product, rent_exemption) => add_to_store(&program_id, accounts, &product, rent_exemption),
